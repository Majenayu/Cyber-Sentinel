...
import {
  Bot, User, Send, Plus, Trash2, MessageSquare, Terminal, Loader2, ChevronRight, ShieldAlert
} from 'lucide-react';
...
export default function ChatPage() {
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { id: 'err', role: 'assistant', content: 'System Error: Failed to establish connection with AI node.' }]);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="flex h-full overflow-hidden font-mono text-sm">
      <div className="w-72 border-r border-border bg-card/20 flex flex-col shrink-0">
        <div className="p-4 border-b border-border bg-black/20 flex items-center justify-between">
          ...
          <button onClick={createSession} className="p-1 hover:bg-primary/20 text-primary border border-primary/30 rounded transition-colors">
            <Plus size={16} />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {isSessionsLoading ? (
            ...
            <button onClick={createSession} className="w-full p-4 border border-dashed border-border rounded text-muted-foreground hover:border-primary/50 hover:text-primary transition-all text-xs flex flex-col items-center gap-2">
              <Terminal size={24} className="opacity-20" /> NO_ACTIVE_SESSIONS
            </button>
          ) : (
            sessions.map(s => (
              <div key={s.id} onClick={() => setCurrentSessionId(s.id)}
                ...
                <button onClick={(e) => deleteSession(s.id, e)} className="opacity-0 group-hover:opacity-100 p-1 hover:text-destructive transition-all">
                  <Trash2 size={12} />
                </button>
              </div>
            ))
      ...
      <div className="flex-1 flex flex-col bg-background/50 relative">
        ...
        <div className="p-4 border-b border-border bg-card/10 flex items-center justify-between z-10">
          ...
          <div className="flex items-center gap-4 text-[10px] text-muted-foreground">
            ...
            <span className="flex items-center gap-1"><span className="h-1.5 w-1.5 rounded-full bg-green-500 inline-block" /> Connected</span>
          </div>
        </div>

        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {messages.length === 0 && !isLoading ? (
            <div className="h-full flex flex-col items-center justify-center text-muted-foreground space-y-4 opacity-50 min-h-[400px]">
            ...
            messages.map((m) => (
              <div key={m.id} className={cn("flex gap-4 max-w-4xl mx-auto", m.role === 'user' ? "flex-row-reverse" : "")}>
                <div className={cn("h-8 w-8 rounded flex items-center justify-center border shrink-0",
                  m.role === 'user' ? "bg-secondary/50 border-primary/20 text-primary" : "bg-black border-primary/40 text-primary shadow-[0_0_10px_rgba(0,255,102,0.1)]"
                )}>
                  {m.role === 'user' ? <User size={16} /> : <Bot size={16} />}
                </div>
                <div className={cn("flex flex-col space-y-2", m.role === 'user' ? "items-end" : "items-start")}>
                  <div className={cn("p-4 rounded-lg border leading-relaxed text-sm",
                    m.role === 'user' ? "bg-secondary/30 border-border text-foreground" : "bg-black/40 border-primary/20 text-foreground shadow-[inset_0_0_20px_rgba(0,0,0,0.5)]"
                  )}>
        ...
        <div className="p-6 border-t border-border bg-card/5 space-y-4 z-10">
          ...
          <div className="max-w-4xl mx-auto flex items-center justify-between text-[10px] text-muted-foreground px-2">
            <div className="flex gap-4">
              <span className="flex items-center gap-1"><div className="h-1 w-1 bg-primary rounded-full" /> ENTER TO SEND</span>
